...
export default {
  ...
  methods: {
    ...
    getQuestion() {
      ...
      getQues({}).then((res) => {
        ...
        if (res.code == 1) {
          ...
          // 当前答案列表
          // this.indexAnswer = this.indexQuestion.option; // 多选
          this.indexAnswer = res.data.questionList[0].option; // 单选

          // 是否选中状态
          this.indexAnswer.forEach((item) => {
            item.select = false;
          });
        } else {
          //未登录
    ...
    // 选择答案 传入答案下标、答案
    selectItem(index, answer) {
      //单选题处理
      if (this.indexQuestion.typedata == 1) {
        let old = this.indexAnswer; //拿到全部选项
        this.indexAnswer = [];
        old.forEach((item, inde) => {
          if (index == inde) {
            item.select = true;
          } else {
            item.select = false;
          }
          this.indexAnswer.push(item);
        });
      } else {
        // 多选题
        let old = this.indexAnswer;
        this.indexAnswer = [];
        old.forEach((item, inde) => {
          if (index == inde) {
            if (item.select) {
      ...
      this.indexAnswer.forEach((item) => {
        if (item.select) {
          this.answerList.push(item.option);
        }
      });
    ...
    confirm() {
      ...
      upAnsw({
        ...
        user_answ: this.answerList.join("|"),
      }).then((res) => {
        Toast.clear();
        console.log(res);
        if (res.code == 1) {
          // 答对数量 correct_number + 1
          ...
          if (res.data.questionSingle.is_correct == 1) {
            ...
            this.isTrue = true;
            this.correct_number++;
            this.right_answer = res.data.questionSingle.answ;
          } else {
            console.log();
    ...
    nextQues() {
      this.index++;
      this.indexQuestion = this.questionList[this.index]; // 单选
      // 当前答案列表
      this.indexAnswer = this.questionList[this.index].option; // 单选
      // 是否选中状态
      this.indexAnswer.forEach((item) => {
        item.select = false;
      });
